...
if __name__ == '__main__':
    ...
    xvals = np.arange(20)
    yvals = [np.ones((xvals.size,)) * i for i in range(5)]
    data = np.stack([xvals] + yvals, axis=1)

    multi_c = MultiCurve.new().from_array(data).on(fig)

    update_btn = pn.widgets.Button(name='New data')
    update_one_btn = pn.widgets.Button(name='Single data')

    def new_data(e):
        xvals = np.arange(20)
        yvals = [(np.random.uniform(size=(xvals.size,)) - 0.5) + i for i in range(5)]
        data = np.stack([xvals] + yvals, axis=1)
        multi_c.update(array=data)

    def new_one_data(e):
        yvals = (np.random.uniform(size=(xvals.size,)) - 0.5) + np.random.randint(0, 6)
        multi_c.update(y_2=yvals)
        # children = multi_c.curves
        # which = np.random.randint(0, len(children))
        # children[which].update(yvals=yvals)

    pointset = PointSet.new().from_vectors(
        x=np.random.randint(0, 10, size=(10,)),
        y=np.random.randint(0, 10, size=(10,)),
    ).on(fig2).make_editable()

    diskset = DiskSet.new().from_vectors(
        x=np.random.randint(0, 10, size=(10,)),
        y=np.random.randint(0, 10, size=(10,)),
        radius=2,
    ).on(fig2)
    ...
    update_points_btn = pn.widgets.Button(name='New points')
    update_points_btn.on_click(update_points)

    def update_radii(e):
        diskset.update(
            radius=np.random.uniform() * 5.,
        )

    update_rad_btn = pn.widgets.Button(name='New radius')
    update_rad_btn.on_click(update_radii)

    remove_btn = pn.widgets.Button(name='Remove')
    add_btn = pn.widgets.Button(name='Add')
    ...
    update_btn.on_click(new_data)
    update_one_btn.on_click(new_one_data)
    remove_btn.on_click(remove_el)
    add_btn.on_click(add_el)

    # Can init direct from another pointset ??
    # PointSet.from_other(
    #     pointset,
    #     defaults=,
    #     extra_defaults=,
    ...
    # pointset2 = PointSet(
    #     pointset.cds,
    #     extra_defaults={'points': dict(fill_color='blue')}
    # )
    # pointset2.on(fig3)

    pn.serve(pn.Row(fig, fig2, pn.Column(update_btn,
                                         update_one_btn,
                                         remove_btn,
                                         add_btn,
                                         update_points_btn,
